...
const Carousel = () => {
	...
	const changeSlideDot = () => {
		const oneSlide = slides.filter(
			(oneSlide, indexSlide) => oneSlide.id === indexSlide
		);
		console.log('slide', oneSlide);
		//setCurrentIndex(oneSlide);
	};
	return (
		<div className='max-w-[1600px] h-[780px] w-full m-auto py-16 relative group'>
			...
			<div className='flex top-4 justify-center py-2'>
				{slides.map((slide, slideIndex) => (
					<div className='text-2xl cursor-pointer' onClick={changeSlideDot}>
						<RxDotFilled />
					</div>
				))}
			</div>
